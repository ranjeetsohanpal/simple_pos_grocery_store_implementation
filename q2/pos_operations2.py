...
def edit_groceries(grocery_lookup : dict): # 2(7)
    '''Lets the users edit stock/price of the grocery item'''

    display_groceries(grocery_lookup)

    print("Edit the grocery stock/price")
    grocery_id = input("Enter grocery id:")

    
    stock_change_check = input("Change stock quantity [y/n]:")
    if stock_change_check.lower() == 'y':
        new_stock = input("Enter new quantity:")
    else:
        new_stock = grocery_lookup[grocery_id]['stock'] #keep the same value 
        
    price_change_check = input("Change item price [y/n]:")
    if price_change_check.lower() == 'y':
        new_price = input("Enter new price:")
    else:
        new_price = grocery_lookup[grocery_id]['price']  

    #confirmation only if the price and/or stock changed
    if stock_change_check.lower() == 'y' or price_change_check.lower() == 'y': 
        confirm = input("Confirm changes [y/n]:")
        if confirm.lower() == 'y':
             grocery_lookup[grocery_id]['stock'] = new_stock
             grocery_lookup[grocery_id]['price'] = new_price
             print("Confirmed changes made")
